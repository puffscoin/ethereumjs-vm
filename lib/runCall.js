...
module.exports = function (opts, cb) {
  ...
  function loadToAccount (done) {
    ...
    if (!toAddress) {
      ...
      if (salt) {
        createdAddress = toAddress = puffsUtil.generateAddress2(caller, salt, code)
      } else {
        createdAddress = toAddress = puffsUtil.generateAddress(caller, newNonce.toArray())
      }
  ...
  function checkAccountState (address, next, done) {
    stateManager.getAccount(address, function (err, account) {
      ...
      if ((account.nonce && new BN(account.nonce) > 0) || account.codeHash.compare(puffsUtil.KECCAK256_NULL) !== 0) {
        toAccount = account
  ...
  function subTxValue (cb) {
    ...
    stateManager.putAccount(puffsUtil.toBuffer(caller), account, cb)
  }
  ...
  function addTxValue (cb) {
    ...
    if (newBalance.gt(puffsUtil.MAX_INTEGER)) {
      cb(new Error('Value overflow'))
      return
    }
    toAccount.balance = newBalance
    // putAccount as the nonce may have changed for contract creation
    stateManager.putAccount(puffsUtil.toBuffer(toAddress), toAccount, cb)
  }
      ...
      if (err) {
        ...
        stateManager.revert(function (revertErr) {
          if (revertErr || !isCompiled) cb(revertErr)
            ...
            // because the bug in both Gpuffs and Parity led to deleting RIPEMD precompiled in this case
